...
const Navbar = () => {
  ...
  const handleHamburger = () => setIsMenuOpen(!isMenuOpen);
      ...
      <div className="navbar max-w-screen-2xl h-16 bg-white z-50 border sticky top-0  shadow-md">
        <div className="content lg:flex lg:justify-between   ">
          ...
          <div className={`options ${isMenuOpen ? 'block' : 'hidden'} lg:block`}>
            ...
            <li className="lg:hidden lg:hover:text-blue-800 font-serif hover:text-black lg:pt-0 pt-7 lg:my-0 mt-3 lg:mx-0 mx-7"><Link to="/home" onClick={closeMenu}>Home</Link></li>
                ...
                <li>
                {user ? (
                  <button type="button" onClick={handlelogout} className="font-serif px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm -mt-2 mt-3 lg:mx-0 mx-7">Logout</button>
                ) : (
                  <button type="button" onClick={handlelogin} className="font-serif lg:w-[90px] w-[130px] h-10 rounded-lg bg-blue-600 hover:bg-blue-700 text-white tracking-wide -mt-2 mt-3 lg:mx-0 mx-7">Sign In</button>
                )}
              </li>
